...
def generate_unique_code():
    ...
    Continuously generate a new code until it is unique within the Room model.
...
class Room(models.Model):
    code = models.CharField(max_length=8, default="", unique=True)  # Unique room code
    host = models.CharField(
        max_length=50, unique=True
    )  # Unique identifier for the host
    guest_can_pause = models.BooleanField(
        null=False, default=False
    )  # Whether guests can pause the room
    votes_to_skip = models.IntegerField(
        null=False, default=1
    )  # Number of votes required to skip
    created_at = models.DateTimeField(
        auto_now_add=True
    )  # Timestamp for when the room is created
